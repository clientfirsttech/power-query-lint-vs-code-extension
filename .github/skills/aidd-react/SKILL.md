...
React components live in the `components/` layer per [structure](../../../apm_modules/kerski/aidd/.apm/skills/aidd-structure/SKILL.md). Consume Observe and void actions from plugins per [service](../../../apm_modules/kerski/aidd/.apm/skills/aidd-service/SKILL.md). Use `@adobe/data-react` for `useDatabase`, `useObservableValues`, and `DatabaseProvider`.
...
**Observe only what you need** — the minimal values required for rendering. For values that may resolve slowly, wrap with `Observe.withDefault` so you can render a skeleton (or placeholder) immediately while waiting. See [observe](../../../apm_modules/kerski/aidd/.apm/skills/aidd-observe/SKILL.md).
