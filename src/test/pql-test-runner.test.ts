...
// ---------------------------------------------------------------------------
// PBI_LOCAL_HOST environment variable (dynamic hostname)
// ---------------------------------------------------------------------------

describe('PBI_LOCAL_HOST environment variable', () => {
  it('getLocalPbiHost returns "localhost" by default', () => {
    const orig = process.env['PBI_LOCAL_HOST'];
    try {
      delete process.env['PBI_LOCAL_HOST'];
      assert.equal(getLocalPbiHost(), 'localhost');
    } finally {
      if (orig === undefined) {
        delete process.env['PBI_LOCAL_HOST'];
      } else {
        process.env['PBI_LOCAL_HOST'] = orig;
      }
    }
  });

  it('getLocalPbiHost returns the PBI_LOCAL_HOST environment variable value', () => {
    const orig = process.env['PBI_LOCAL_HOST'];
    try {
      process.env['PBI_LOCAL_HOST'] = 'my-dev-box';
      assert.equal(getLocalPbiHost(), 'my-dev-box');
    } finally {
      if (orig === undefined) {
        delete process.env['PBI_LOCAL_HOST'];
      } else {
  ...
  it('resolveXmlaConnection uses PBI_LOCAL_HOST in the local port server string', () => {
    // Arrange: create a fake LOCALAPPDATA with the full PBI Desktop workspace path
    const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'pql-localdata-'));
    const wsDir = path.join(
      tmpRoot,
      'Microsoft',
      'Power BI Desktop',
      'AnalysisServicesWorkspaces',
      'AnalysisServicesWorkspace44444',
      'Data'
    );
    fs.mkdirSync(wsDir, { recursive: true });
    fs.writeFileSync(path.join(wsDir, 'msmdsrv.port.txt'), '44444', 'utf8');

    // Create a model dir with a .pbip file so the local detection path is taken
    const modelDir = fs.mkdtempSync(path.join(os.tmpdir(), 'pql-model-'));
    fs.writeFileSync(path.join(modelDir, 'MyModel.pbip'), '{}', 'utf8');

    const origLocalAppData = process.env['LOCALAPPDATA'];
    const origPbiLocalHost = process.env['PBI_LOCAL_HOST'];
    try {
      process.env['LOCALAPPDATA'] = tmpRoot;
      process.env['PBI_LOCAL_HOST'] = 'custom-host';

      const conn = resolveXmlaConnection(modelDir, {});
      assert.ok(conn !== undefined, 'Expected a connection to be resolved');
      assert.ok(
        conn!.server.startsWith('custom-host:'),
        `Expected server to start with "custom-host:" but got "${conn!.server}"`
      );
      assert.equal(conn!.server, 'custom-host:44444');
    } finally {
      ...
      if (origPbiLocalHost === undefined) {
        delete process.env['PBI_LOCAL_HOST'];
      } else {
        process.env['PBI_LOCAL_HOST'] = origPbiLocalHost;
      }
      fs.rmSync(tmpRoot, { recursive: true });
